...
class Mage_Adminhtml_Block_Sales_Order_View extends Mage_Adminhtml_Block_Widget_Form_Container
{
    ...
    public function __construct()
    {
        ...
        /* UPDATE INCLUDED BY PAYDOLLAR PAYMENT MODULE - START */
        /**
         * This button is added for PayDollar for QUERY API button for manual checking of payment result    
         */
        if($order->getPayment()->getMethodInstance()->getCode() == 'pdcptb'){
        	if($order->getData('state') == Mage_Sales_Model_Order::STATE_NEW){        		
        		$message = 'Query the Payment Gateway To Check If Any Payments Made?';
        		$url = Mage::helper('adminhtml')->getUrl('pdcptb/adminhtml_adminpdcptb/query/',array('order_id'=>$order->getId()));
        		$this->addButton('paydollar_query_api', array(
        				'label'     => '[ --- PAYDOLLAR: QUERY PAYMENT STATUS --- ]',
        				'onclick'   => "confirmSetLocation('{$message}', '{$url}')",
        		));        		
        	}
        }        
        /* UPDATE INCLUDED BY PAYDOLLAR PAYMENT MODULE - END */
